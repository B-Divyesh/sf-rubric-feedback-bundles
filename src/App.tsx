...
export function App() {
  ...
  return <div className="app">
    ...
    {!bundles.length ? <Welcome onCreate={create} /> : view === 'grade' && current ? <Workspace bundle={current} onChange={persist} onMessage={announce} /> : view === 'summary' && current ? <Summary bundle={current} unlocked={license.unlocked} onMessage={announce} /> : view === 'bundles' ? <BundleLibrary bundles={bundles} currentId={current?.id ?? ''} unlocked={license.unlocked} onOpen={(id) => { setCurrentId(id); location.hash = 'grade'; }} onCreate={create} onDelete={remove} onMessage={announce} /> : <Settings license={license} onLicense={checkLicense} onImport={restore} onExport={backup} onMessage={announce} />}
    <AppFooter />
